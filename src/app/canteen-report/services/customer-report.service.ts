...
export class CustomerReportService {
  private apiUrl = 'https://canteen.benzyinfotech.com/api/v3/customer/report';
  private token =
  ...
  customerCanteenReport(monthsOrderData: MonthsOrder): Observable<User> {
    const headers = new HttpHeaders({
      'Content-Type': 'application/json',
      Authorization: `Bearer ${this.token}`,
    });
    return this._http.post<User>(this.apiUrl, monthsOrderData, {
      headers: headers,
    });
  }
}
